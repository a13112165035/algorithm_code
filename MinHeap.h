...
template<class T>MinHeap<T>::MinHeap(int n){	
	MaxSize=n;	
...
template<class T>void MinHeap<T>::FilterUp(int start) //自下往上调整
{	
...
template<class T>void MinHeap<T>::FilterDown(const int start,const int end) //自上往下调整，使关键字小的节点在上
{	
	int i=start,j=2*i+1;	
	T temp=heap[i];	
while(j<=end)	
...
template<class T>bool MinHeap<T>::IsFull() const{	
